...
import { PROJECT_RECOMMEND } from '../../database';
...
const ProjectRecommend = () => {
  const randomProjects = getRandomProjects(PROJECT_RECOMMEND, 3); // 랜덤으로 3개 프로젝트 선택
      ...
      <S.ProjectCardContainer>
        {randomProjects.map((project) => (
          <S.ProjectCard key={project.id}>
            <S.ProjectCardImage>
              <img
                src={`/images/thumbnails/${project.imageUrl}`}
                alt={project.name}
              />
            </S.ProjectCardImage>
          </S.ProjectCard>
